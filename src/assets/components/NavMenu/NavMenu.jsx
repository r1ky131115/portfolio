...
import { Menu2, LetterX } from 'tabler-icons-react';
      ...
      >
        <section className="u-center flex justify-between h-16 items-center">
          <div className="flex items-center gap-2">
            ...
            >
              {menuOpen ? (
                <LetterX className="w-10 h-10" />
              ) : (
                <Menu2 className="w-10 h-10" />
              )}
            </button>
            <a
              href="#Home"
              ...
              {navLinks.map((link) => {
                return (
                  <a
                    href={link.href}
                    key={link.id}
                    className="h-full flex text-lg items-center p-2 hover:bg-[var(--hover-nav)] hover:border-b-2 hover:border-[var(--foreground)] text-[var(--text-nav)] hover:text-[var(--foreground)]"
                  >
                    <li key={link.id}>{link.text}</li>
                  </a>
                );
